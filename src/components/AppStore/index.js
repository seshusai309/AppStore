...
class AppStore extends Component {
  state = {tabs: tabsList[0].tabId, searched: ''}

  changedTab = id => {
    this.setState(each => ({tabs: id, searched: each.searched}))
  }

  searchInputResult = event => {
    this.setState(each => ({tabs: each.tabs, searched: event.target.value}))
  }

  render() {
    const {tabs, searched} = this.state
    const filteredSearch = appsList.filter(each => each.category === tabs)
    const finalFilter = filteredSearch.filter(each =>
      each.appName.toLowerCase().includes(searched.toLowerCase()),
    )
    return (
      <div className="container">
        <h1 className="heading">App Store</h1>

        <div className="seachedInput">
          <input
